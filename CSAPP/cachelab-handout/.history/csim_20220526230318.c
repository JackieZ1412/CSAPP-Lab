...
void cache_option(int op,long addr,int size,int time)
{
    ...
    else if(op == 2 || op == 4){   // S_type or M_type need to write to the cache
        int hit = 0;
        for(int i = 0;i < E;i++){
            if(cache[tag][i].tag == tag){   //hit
                cache[tag][i].last_visited = 0;
                hit_count++;
                hit = 1;
                break;
            }
            if(hit == 0){
                ...
                if(sign == 0){
                    ...
                    for(int i = 0;i < E;i++){
                        if(cache[tag][i].last_visited > max_last_visited){
                            max_last_visited = cache[tag][i].last_visited;
                            flag = i;
                        }
                    }
                    cache[tag][flag].last_visited = 0;
                    cache[tag][flag].tag = 
                }
            }
        }
